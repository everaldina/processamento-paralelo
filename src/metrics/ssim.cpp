...
namespace metrics {
    ...
    double calculate_ssim(const std::vector<unsigned char>& im1, const std::vector<unsigned char>& im2, int width, int height, int win_size) {
        ...
        const double L = 255.0; // Dynamic range para 8-bit unsigned
        ...
        // Itera pixel a pixel, aplicando a janela (ignorando as bordas onde a janela não caberia perfeitamente)
        for (int y = offset; y < height - offset; ++y) {
            for (int x = offset; x < width - offset; ++x) {
                ...
                // Variâncias e Covariância (fórmula de população, conforme clássico paper Wang 2004)
