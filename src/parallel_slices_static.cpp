...
void process_image_pair(const std::string& image_id, const std::string& pathA, const std::string& pathB, 
    ...
    #pragma omp parallel num_threads(numThreads)
    {
        ...
        if (size_parallel > 0) {
            #pragma omp for nowait reduction(+:total_ssim_time, ssim_calc_count) schedule(static, size_parallel)
            ...
            #pragma omp for nowait reduction(+:total_ssim_time, ssim_calc_count) schedule(static)
            for (int i = 0; i < cortes_B; ++i) {
                const short* ptr_slice_B = volume_B.data() + (i * pixels_per_slice);
...
int main(int argc, char* argv[]) {
    ...
    std::string file_name = "slices_static" + std::to_string(size_parallel) + 
