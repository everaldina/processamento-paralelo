...
- A janela deslizante (media, variancia, covariancia, numerador/denominador do SSIM) roda em `float`, nao `double` - GPUs GeForce de consumidor (como a GTX 1060) rodam `double` a 1/32 da velocidade de `float` de proposito (segmentacao de mercado da NVIDIA), e essa e a parte mais cara do kernel (roda win_size*win_size vezes por pixel), entao e onde compensa usar float. Isso significa que essa parte NAO e mais identica bit a bit a CPU (que usa double): a soma dos pixels de uma janela ainda cabe exata em float, mas a variancia (uma subtracao de dois numeros grandes e proximos) pode perder precisao de forma mais visivel. Ver o comentario de precisao no topo de `src/metrics/ssim_kernel.cuh` para mais detalhes.
- Ja a soma final do SSIM de todos os pixels de um corte continua em `double` e e feita em paralelo (reducao em arvore dentro do bloco), enquanto a CPU faz essa soma sequencialmente. Soma de ponto flutuante nao e associativa, entao o SSIM de um corte pode diferir da CPU tanto pela reducao em arvore quanto pela mudanca pra float na janela. E uma caracteristica esperada da otimizacao, nao um bug - mas por isso vale reconferir a corretude (proximo item) depois dessa mudanca especificamente, em vez de so assumir que a margem de erro continua tao pequena quanto antes.
